...
export function resolveDrawableTarget(iconPath: string): DrawableTarget {
  ...
  if (directory === undefined) {
    throw new Error(
      `[@rn-media/media-session] \`androidNotificationIcon\` must be a ${Object.keys(
  ...
  if (!isValidAndroidAssetName(resourceName)) {
    throw new Error(
      `[@rn-media/media-session] "${resourceName}" is not a valid Android resource name. ` +
  ...
  withDangerousMod(config, [
    ...
    async (dangerousConfig) => {
      ...
      if (!existsSync(source)) {
        throw new Error(
          `[@rn-media/media-session] \`androidNotificationIcon\` points at "${iconPath}", ` +
