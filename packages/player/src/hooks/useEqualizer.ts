...
export function useEqualizer(
  ...
  useEffect(() => {
    ...
    try {
      ...
      // `@afkcodes/timbre-media-session`'s `withPersistence` uses, for the same reason.
...
function reportStorageError(
  ...
  console.warn(
    '[@afkcodes/timbre-player] useEqualizer: the storage adapter failed. Pass `onStorageError` to handle it.',
    cause
  )
}
