# Spec: `@afkcodes/timbre-audio-session`
...
- Package `packages/audio-session`, npm `@afkcodes/timbre-audio-session`.
- **Singleton** JS API (the OS session/focus is singular — CLAUDE.md principle 5 exception).
- Zero dependency on `@afkcodes/timbre-player` and vice versa. Integration is an explicit
...
```ts
import { AudioSession } from '@afkcodes/timbre-audio-session';
...
```ts
import { wireAudioSession } from '@afkcodes/timbre-audio-session';
