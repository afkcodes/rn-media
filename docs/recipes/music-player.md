...
```tsx
import { useEqualizer, type Player } from '@afkcodes/timbre-player'
...
| `playbackResumption` needs a manifest half | media3's `MediaButtonReceiver`, or `["@afkcodes/timbre-media-session", { "playbackResumption": true }]` under Expo prebuild |
