...
| **Offline waveform / peaks extraction** (whole-file analysis for a static waveform) | Not a playback-time concern; needs a decode-faster-than-realtime path libmpv's client API does not offer. Belongs in `@rn-media/downloads` or a separate tool. Deliberate-no *for `player`*; note as a downloads-package candidate. |
