...
- **`@afkcodes/timbre-video` plugin contract**: the Player HybridObject exposes a narrow
...
- `@afkcodes/timbre-player` — multi-instance libmpv Player; typed state/reducer;
  ...
  tests.
- `@afkcodes/timbre-audio-session` — focus/AVAudioSession arbiter, interruption/
  noisy/route events, music/speech presets, `wireAudioSession`. 41 tests.
- `@afkcodes/timbre-media-session` — media3 1.11 MediaLibraryService +
...
around mpv, not an mpv output — `@afkcodes/timbre-cast`; see docs/design/cast.md),
...
3. **`@afkcodes/timbre-downloads`** — offline playback package. Design key: it ships
...
5. **Investigations**: ~~pitch control via an LGPL filter path~~ — **CLOSED,
   ...
   **URL handoff**, not an output route — the engine is *paused* for the
   ...
   sender SDK exists for third-party engines. Shipped as `@afkcodes/timbre-cast` —
   see item 6.
6. **Casting — design APPROVED 2026-08-13; the item-5 deferral is rewritten
   ...
   is the owner-approved design: first-party `@afkcodes/timbre-cast` binding over the
   ...
   Phase 1 sign-off DONE; Phase 2 — the `@afkcodes/timbre-cast` package (Nitro
   ...
   `wireCastHandoff`, living in **@afkcodes/timbre-cast, not media-session** (which
...
first-party via `@afkcodes/timbre-cast` on both platforms (docs/design/cast.md); an
